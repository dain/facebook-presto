        ...
        implements StageScheduler
{
    ...
    private CompletableFuture<List<Split>> futureSplits;
    ...
    {
        // acquire the future state change before doing calculations, se if we need
        // to return pending results, we will wake up immediately for changes during
        // the calculation
        ...
        if (pendingSplits.isEmpty()) {
            if (futureSplits == null) {
                ...
                futureSplits = splitSource.getNextBatch(splitBatchSize);
                long start = System.nanoTime();
                futureSplits.thenRun(() -> stage.recordGetSplitTime(start));
            }

            if (!futureSplits.isDone()) {
                return new ScheduleResult(false, ImmutableSet.of(), futureSplits);
            }
            pendingSplits = ImmutableSet.copyOf(getFutureValue(futureSplits));
            futureSplits = null;
        }
