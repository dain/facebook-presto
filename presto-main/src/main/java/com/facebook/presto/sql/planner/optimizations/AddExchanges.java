...
import static com.facebook.presto.sql.planner.SystemPartitionFunctionHandle.ROUND_ROBIN;
            ...
            extends PlanVisitor<Context, PlanWithProperties>
    {
        ...
        public PlanWithProperties visitJoin(JoinNode node, Context context)
        {
            ...
            if ((distributedJoins && !(node.getType() == INNER && leftSymbols.isEmpty())) || node.getType() == FULL || node.getType() == RIGHT) {
                ...
                if (!left.getProperties().isDistributedOn(leftSymbols) || (left.getProperties().isEffectivelySinglePartition() && left.getProperties().isRepartitionEffective(leftSymbols))) {
